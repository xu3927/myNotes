...
- lax 默认值, 允许 top-level 跨域请求携带cookie. top-level 请求包含 <a href=""></a> <link rel="prerender" /> <form method="GET"> 
- strict 只允许同域
- none 不做限制, 跨域也可以发送. 如果设置为none, 同时需要设置 secure 属性值

#### Secure

设置为 secure 表示只能通过https发送请求. http请求的cookie不能设置为 secure 
