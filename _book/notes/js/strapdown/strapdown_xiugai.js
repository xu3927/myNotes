...
(function() {
            ...
            if (cap = this.rules.list.exec(src)) {
                ...
                for (; i < l; i++) {
                    ...
                    if (this.options.smartLists && i !== l - 1) { b = block.bullet.exec(cap[i + 1])[0];
                        if (bull !== b && !(bull[1] === '.' && b[1] === '.')) { src = cap.slice(i + 1).join('\n') + src;
                            i = l - 1; } }
                    loose = next || /\n\n(?!\s*$)/.test(item);
            ...
            case 'code':
                {
                    if (this.options.highlight) {
                        ...
                        if (code != null && code !== this.token.text) { this.token.escaped = true;
    ...
    function marked(src, opt) {
        try {
            ...
            return Parser.parse(Lexer.lex(src, opt), opt); } catch (e) {
    ...
    if (typeof exports === 'object') { module.exports = marked; } else if (typeof define === 'function' && define.amd) { define(function() {
            return marked; }); } else { this.marked = marked; }
}).call(function() {
...
(function() {
    ...
    function combinePrefixPatterns(regexs) {
        ...
        for (var i = 0, n = regexs.length; i < n; ++i) {
            var regex = regexs[i];
            if (regex.ignoreCase) { ignoreCase = true; } else if (/[a-z]/i.test(regex.source.replace(/\\u[0-9a-f]{4}|\\x[0-9a-f]{2}|\\[^ux]/gi, ''))) { needToFoldCase = true;
                ignoreCase = false;
                break; } }
        ...
        function decodeEscape(charsetPart) {
            ...
            if (cc0) {
                return cc0; } else if ('0' <= c1 && c1 <= '7') {
                return parseInt(charsetPart.substring(1), 8); } else if (c1 === 'u' || c1 === 'x') {
                return parseInt(charsetPart.substring(2), 16); } else {
        ...
        function caseFoldCharset(charSet) {
            ...
            for (var i = 0; i < ranges.length; ++i) {
                var range = ranges[i];
                if (range[0] <= lastRange[1] + 1) { lastRange[1] = Math.max(lastRange[1], range[1]); } else { consolidatedRanges.push(lastRange = range); } }
        ...
        function allowAnywhereFoldCaseAndRenumberGroups(regex) {
            ...
            for (var i = 0, groupIndex = 0; i < n; ++i) {
                ...
                if (p === '(') {++groupIndex; } else if ('\\' === p.charAt(0)) {
                    ...
                    if (decimalValue) {
                        if (decimalValue <= groupIndex) { capturedGroups[decimalValue] = -1; } else { parts[i] = encodeEscape(decimalValue); } } } }
            for (var i = 1; i < capturedGroups.length; ++i) {
                if (-1 === capturedGroups[i]) { capturedGroups[i] = ++capturedGroupIndex; } }
            for (var i = 0, groupIndex = 0; i < n; ++i) {
                var p = parts[i];
                if (p === '(') {++groupIndex;
                    if (!capturedGroups[groupIndex]) { parts[i] = '(?:'; } } else if ('\\' === p.charAt(0)) {
                    var decimalValue = +p.substring(1);
            ...
            if (regex.ignoreCase && needToFoldCase) {
                for (var i = 0; i < n; ++i) {
                    var p = parts[i];
                    var ch0 = p.charAt(0);
                    if (p.length >= 2 && ch0 === '[') { parts[i] = caseFoldCharset(p); } else if (ch0 !== '\\') { parts[i] = p.replace(/[a-zA-Z]/g, function(ch) {
    ...
    function numberLines(node, opt_startLineNum, isPreformatted) {
        ...
        function walk(node) {
            switch (node.nodeType) {
                case 1:
                    if (nocode.test(node.className)) {
                        break; }
                    if ('br' === node.nodeName) { breakAfter(node);
                        if (node.parentNode) { node.parentNode.removeChild(node); } } else {
                        for (var child = node.firstChild; child; child = child.nextSibling) { walk(child); } }
                    break;
                case 3:
                case 4:
        ...
        function breakAfter(lineEndNode) {
            while (!lineEndNode.nextSibling) { lineEndNode = lineEndNode.parentNode;
                if (!lineEndNode) {
                    return; } }
            ...
            function breakLeftOf(limit, copy) {
                ...
                if (parent) {
                    ...
                    for (var sibling = next; sibling; sibling = next) { next = sibling.nextSibling;
    ...
    function recombineTagsAndDecorations(job) {
        ...
        for (i = decPos = 0; i < nDecorations;) {
            if (decorations[i] !== decorations[i + 2]) { decorations[decPos++] = decorations[i++];
    ...
    function registerLangHandler(handler, fileExtensions) {
        for (var i = fileExtensions.length; --i >= 0;) {
            ...
            if (!langHandlerRegistry.hasOwnProperty(ext)) { langHandlerRegistry[ext] = handler; } else if (win['console']) { console['warn']('cannot override language handler %s', ext); } } }
    ...
    function applyDecorator(job) {
        ...
        try {
            ...
            recombineTagsAndDecorations(job); } catch (e) {
    ...
    function prettyPrint(opt_whenDone) {
        ...
        codeSegments = null;
        var clock = Date;
        if (!clock['now']) { clock = { 'now': function() {
                    return +(new Date); } }; }
        var k = 0;
            ...
            for (; k < elements.length && clock['now']() < endTime; k++) {
                ...
                if (prettyPrintRe.test(className) && !prettyPrintedRe.test(className)) {
                    ...
                    for (var p = cs.parentNode; p; p = p.parentNode) {
                        var tn = p.tagName;
                        if (preCodeXmpRe.test(tn) && p.className && prettyPrintRe.test(p.className)) { nested = true;
                            break; } }
                    if (!nested) {
    ...
    if (typeof define === "function" && define['amd']) { define("google-code-prettify", [], function() {
            return PR; }); }
})();;

(function(window, document) {

	//如果页面内没有markdonw元素则直接return
	// markdown的标记标签
	var markDownTags = [];
	if(document.getElementsByTagName('xmp').length > 0 ){
		for(var i = 0 ; i < document.getElementsByTagName('xmp').length ; i ++ ){

			markDownTags.push(document.getElementsByTagName('xmp')[i]);
	...
	if(document.getElementsByTagName('textarea').length > 0 ){
		for(var i = 0 ; i < document.getElementsByTagName('textarea').length ; i ++ ){
			...
			markDownTags.push(document.getElementsByTagName('textarea')[i]);

		}
	}

	//如果不存在markdown的标记标签则直接return
    ...
    // Get theme  主题 theme在一个 script 标签中设置
    var themeTag = document.getElementById("theme_strapdown");
    var theme = themeTag.getAttribute("theme") || 'bootstrap';
    theme = theme.toLowerCase();

    // Stylesheets
    var linkEl = document.createElement('link');
    //bootsrap 样式文件
    linkEl.href = 'js/strapdown/bootswatch-theme/' + theme + '.min.css';
    linkEl.rel = 'stylesheet';
    document.head.appendChild(linkEl);

    var linkEl = document.createElement('link');
    linkEl.href = 'js/strapdown/strapdown.css';
    linkEl.rel = 'stylesheet';
    document.head.appendChild(linkEl);

    var linkEl = document.createElement('link');

    //切换为本地的文件 bootsrap 响应式
    linkEl.href = 'js/strapdown/bootstrap-responsive.min.css';
    linkEl.rel = 'stylesheet';
    document.head.appendChild(linkEl);
    ...
    /////////////////////////////////
     // Insert navbar if there's none 导航条
  ...
  function formatTag (markdownEl, n) {
    ...
    document.getElementById('content' + n).innerHTML = html;
    ...
    // document.body.style.display = ''; 
  	...
  	//调用函数格式化各个markdown标签
  	for(var i = 0 ; i < markDownTags.length ;  i ++ ){

			formatTag(markDownTags[i],i);  //格式化第i个标记标签
